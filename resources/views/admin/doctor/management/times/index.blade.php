                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title mb-3">مدیریت زمان های دکتر ({{$doctor->fullName}})</h3>

                            <div class="card-tools">
                                <form id="filterForm" method="get"
                                      action="{{route('management_doctor_times_index',$doctor['id'])}}">
                            ...
                            <a href="{{route('management_doctor_times_index',[$doctor['id'],'status='.'active'])}}"
                               class="btn btn-success">منقضی نشده(تاریخ)</a>

                            <a href="{{route('management_doctor_times_index',[$doctor['id'],'status='.'not-capacity'])}}"
                               class="btn btn-warning">اتمام ظرفیت</a>

                            <a href="{{route('management_doctor_times_index',[$doctor['id'],'status='.'expire-date'])}}"
                                ...
                                @if(count($data))
                                    ...
                                    @foreach($data as $key=>$value)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a target="_blank"
                                                   href="{{route('management_doctor_times_edit',[$doctor['id'],$value->id])}}">
                                                    <i class="fa fa-edit text-primary"></i>
                                                </a>
                                            </td>
                                            <td>
                                                <a href="{{ route('management_doctor_times_destroy', [$doctor['id'],$value->id]) }}"
                                                   onclick="destroyDoctorTime(event,{{ $doctor['id'] }}, {{ $value->id }})"><i
                                                        class="fa fa-remove text-danger"></i></a>
                                                <form
                                                    action="{{ route('management_doctor_times_destroy', [$doctor['id'],$value->id]) }}"
                                                    method="post"
                                                    id="destroy-DoctorTime-{{ $doctor['id'] }}-{{ $value->id }}">
                                                    @csrf
                                                    @method('delete')
                                                </form>
                                            </td>
                        ...
                        <div class="card-footer">
                            <a target="_blank" class="btn btn-primary"
                               href="{{route('management_doctor_times_create',$doctor['id'])}}">
                                افزودن زمان های دکتر
                                ({{$doctor->fullName}})
                            </a>
                        </div>
...
<script type="text/javascript">
    ...
    $('#filterForm').on('submit', function (e) {
        ...
        var route = "{{route('management_doctor_times_index',$doctor['id'])}}";
    ...
    function destroyDoctorTime(event, doctor_id, id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById(`destroy-DoctorTime-${doctor_id}-${id}`).submit()
            }
        })
    }
